...
| 场景 | 处理方式 |
|------|---------|
| HAR 文件损坏（非 JSON） | 输出明确错误信息，建议用户用浏览器重新导出 |
| HAR 文件过大（>100MB） | 警告并建议按时间范围筛选后重新录制 |
| repo-profiles.yaml 格式错误 | 使用 Pydantic 校验并输出具体字段错误，继续解析（matched_repo 为 null） |
| 所有条目被过滤 | 输出警告，列出被过滤的原因统计（静态资源 N 条、WebSocket N 条等） |
| base64 解码失败 | 跳过该条目的 response.body，记录警告 |
