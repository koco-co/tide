#!/usr/bin/env python3
"""只删除 init-tide 在系统临时根目录中创建的本次专用目录。"""
...
def create():
    return Path(tempfile.mkdtemp(prefix="init-tide-")).resolve()
...
def cleanup(path):
    ...
    if len(relative.parts) != 1 or not relative.name.startswith("init-tide-"):
        raise CleanupError("只允许删除本次 init-tide 专用临时目录。")
...
def parser():
    value = argparse.ArgumentParser(description="安全清理本次 init-tide 系统临时目录。")
    value.add_argument("path", nargs="?", help="init-tide 本次专用临时目录")
