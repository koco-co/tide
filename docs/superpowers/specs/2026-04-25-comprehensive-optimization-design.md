...
将 `skills/autoflow/SKILL.md`（682 行）和 `skills/using-autoflow/SKILL.md`（866 行）瘦身为纯编排骨架，所有细节外移到 `agents/*.md` 和 `scripts/*.py`。
...
- `agents/project-scanner.md` — 保持（仅 using-autoflow）
- `agents/industry-researcher.md` — 保持（仅 using-autoflow）
...
- 下次 `/autoflow` 启动时：读取偏好作为上下文传递给 Agent
...
| `scaffold.py:194-203,286-293` | autoflow-config 渲染重复 | 抽取 `_render_autoflow_config()` |
...
修改:
  ...
  skills/autoflow/SKILL.md             (重写为极简编排)
  skills/using-autoflow/SKILL.md       (重写为极简编排)
  ...
  scripts/scaffold.py                  (抽取 _render_autoflow_config)
